  ...
  with tf.python_io.TFRecordWriter(train_output_name) as writer:
    for idx, pair in enumerate(train_file_pairs):
      ...
      try:
        ...
        pm = pretty_midi.PrettyMIDI(midi_file=pair[1])
        note_starts = []
        note_ends = []
        note_programs = []
        note_pitches = []
        ...
        for instr in pm.instruments:
          for note in instr.notes:
            assert instr.is_drum == False
            note_starts.append(note.start)
            note_ends.append(note.end)
            note_pitches.append(note.pitch)
            note_programs.append(instr.program)
            # TODO: make sure timidity and fluidsynth give the same output
            # TODO: how to find mask relative to original
            pm_iso = pretty_midi.PrettyMIDI()
            pm_iso.instruments.append(instr)
            pm_iso.instruments[0].notes = [note]
            note_wavs.append(pm_iso.fluidsynth(sf2_path=sf2_path))

        # load the wav data
        wav_data = tf.gfile.Open(pair[0], 'rb').read()
        samples = audio_io.wav_data_to_samples(wav_data, FLAGS.sample_rate)
        samples2 = pm.fluidsynth(sf2_path=sf2_path)
        print(len(samples))
        print(len(samples2))
        ...
        for start, end in zip(splits[:-1], splits[1:]):
          ...
          if len(new_samples) < 5*16000 or len(new_samples) > 20*16000:
            continue
          # need to have: note onset, note frame, wav around onset and frame, isolated note sound or mask
          # need to loop over each note in the midi, set some max frame length
